// Shared routing-resource metadata for the proxy/group editors. Mirrors the
// upstream visual config editor's field lists so the same fields are offered
// and the same ones are treated as sensitive.
...
export const NUMBER_ROUTING_FIELDS = new Set(["port", "interval", "mtu", "alterId"]);

export function routingResourceFields(kind: "proxy" | "group", type: string): string[] {
  if (kind === "group") return ["name", "type", "url", "interval"];
  return ["name", "type", "server", "port", ...(PROXY_PROTOCOL_FIELDS[type] ?? [])];
}

export function isSensitiveRoutingField(field: string): boolean {
